...
def get_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, f_pool_size, t_pool_size,
    ...
    # CNN
    # model definition
    spec_start = Input(shape=(data_in[-3], data_in[-2], data_in[-1]))  # (ch, time, mel)

    # Convert to channels_last: (time, mel, ch)
    spec_cnn = Permute((2, 3, 1))(spec_start)

    # CNN (now channels_last)
    for i, convCnt in enumerate(f_pool_size):
        spec_cnn = Conv2D(filters=nb_cnn2d_filt, kernel_size=(3, 3), padding='same')(spec_cnn)
        spec_cnn = BatchNormalization()(spec_cnn)  # default axis=-1 is correct for channels_last
        spec_cnn = Activation('relu')(spec_cnn)
        spec_cnn = MaxPooling2D(pool_size=(t_pool_size[i], f_pool_size[i]))(spec_cnn)
        spec_cnn = Dropout(dropout_rate)(spec_cnn)

    # RNN input: (time, features)
