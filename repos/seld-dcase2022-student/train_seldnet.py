...
def load_ckpt_with_pruning(model, ckpt_path):
    import torch
    state = torch.load(ckpt_path, map_location='cpu')

    # If checkpoint was saved with torch.nn.utils.prune reparameterization
    if any(k.endswith("weight_orig") for k in state.keys()):
...
def maybe_dynamic_quantize(model, params):
    if not params.get('dynamic_quant', False):
        return model

    # Dynamic quantization works well for RNN/Linear layers
...
def build_teacher_model(data_in, data_out, params, device):
    teacher_params = dict(params)

    # Teacher architecture
    teacher_params['nb_cnn2d_filt'] = 64
    teacher_params['nb_rnn_layers'] = 2
    teacher_params['rnn_size'] = 128
    teacher_params['nb_fnn_layers'] = 1
    teacher_params['fnn_size'] = 128

    teacher = seldnet_model.CRNN(data_in, data_out, teacher_params).to(device)

    teacher_state = torch.load(params['teacher_model_path'], map_location=device)
...
def train_epoch(data_generator, optimizer, model, criterion, params, device, teacher_model=None):
    ...
    for data, target in data_generator.generate():
        ...
        if params.get('distill', False) and teacher_model is not None:
            with torch.no_grad():
                teacher_output = teacher_model(data)
            kd_loss = nn.MSELoss()(output, teacher_output)
            alpha = params.get('kd_alpha', 0.5)
...
def main(argv):
    ...
    # Training setup
    # ---------------- Split selection (patched for local Windows paths) ----------------
    train_splits, val_splits, test_splits = None, None, None

    if params['mode'] == 'dev':
        # Use unique_classes to determine the dataset edition instead of string-matching dataset_dir
        if params['unique_classes'] == 14:  # DCASE2020
            test_splits = [1]
            val_splits = [2]
            train_splits = [[3, 4, 5, 6]]

        elif params['unique_classes'] == 12:  # DCASE2021
            test_splits = [6]
            val_splits = [5]
            train_splits = [[1, 2, 3, 4]]

        elif params['unique_classes'] == 13:  # DCASE2022
            test_splits = [[4]]
            val_splits = [[4]]
            train_splits = [[1, 2, 3]]

        else:
            raise ValueError(f"ERROR: Unknown dataset setup: unique_classes={params['unique_classes']}")
    # ---------------------------------------------------------------------------------
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        if params.get('distill', False):
            teacher_model = build_teacher_model(data_in, data_out, params, device)
        model = load_ckpt_with_pruning(model, params['pretrained_model_weights'])
        if params.get('do_prune', False):
            model = apply_global_pruning(
                model,
                amount=params.get('prune_amount', 0.3)
            )
        ...
        if params['finetune_mode']:
            ...
            if any(k.endswith("weight_orig") for k in state.keys()):
                ...
                # Create the same reparameterization structure in the current model
                for _, module in model.named_modules():
                    if hasattr(module, "weight") and isinstance(module.weight, torch.Tensor):
                        try:
                            prune.identity(module, "weight")
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode='min',          # because lower SELD is better
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            train_loss = train_epoch(data_gen_train, optimizer, model, criterion, params, device, teacher_model)
        ...
        # ---------------------------------------------------------------------
        if params['nb_epochs'] > 0:
            print('Load best model weights')
            model = load_ckpt_with_pruning(model, model_name)
        else:
            print('Skipping best-weight reload (nb_epochs=0). Using pretrained weights.')
        model.eval()

        # ---- remove pruning re-param before quantization / saving ----
        # Remove pruning reparameterization if present (weight_orig/weight_mask)
        ...
        model = maybe_dynamic_quantize(model, params)
        # If quantized, run evaluation on CPU
        if params.get('dynamic_quant', False):
