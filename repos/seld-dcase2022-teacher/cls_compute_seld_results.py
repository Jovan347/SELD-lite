...
class ComputeSELDResults(object):
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False):
        ...
        if is_jackknife:
            ...
            return [ER, conf_interval[0]], [F, conf_interval[1]], [LE, conf_interval[2]], [LR, conf_interval[3]], [seld_scr, conf_interval[4]], [classwise_results, np.array(conf_interval)[5:].reshape(5, self._unique_classes, 2) if len(classwise_results) else []]
...
if __name__ == "__main__":
    pred_output_format_files = r"results\3_1_dev_split0_multiaccdoa_foa_20260108072745_test" # Path of the DCASEoutput format files
    argv = sys.argv[1] if len(sys.argv) > 1 else '1'
    params = parameters.get_params(argv)

    # (optional but safe)
    params.setdefault('unique_classes', 12)
    params.setdefault('nb_classes', params['unique_classes'])
